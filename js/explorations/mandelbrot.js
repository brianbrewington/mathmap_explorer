...
$$z_{n+1} = z_n^2 + c, \\quad z_0 = 0,\\; c \\in \\mathbb{C}$$
</div>
<p>Point $c$ is <em>in</em> the set if $|z_n|$ remains bounded as $n \\to \\infty$.</p>
<p>For each point $c$ in the complex plane, we iterate $z \\to z^2 + c$ starting from $z = 0$. If the orbit stays bounded ($|z| \\leq 2$), $c$ belongs to the Mandelbrot set. The boundary reveals an infinitely complex fractal.</p>`;
